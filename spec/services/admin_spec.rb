...
RSpec.describe News::Admin  do
  ...
  let(:expire) { (Time.now + 3.minutes).to_s(:db) }
  ...
  context 'admin news record' do
    it 'checks admin news record save works correctly' do
      check_date = Time.now
      expect { News::Admin.new().save_admin_news_record(data, expire) }
      .to change { ActiveRecord::Base.connection.execute("SELECT * FROM que_jobs").count }
      .from(0).to(1)
      expect(redis.ttl(NewsRoot::ADMIN_NEWS_KEY) <= 3.minutes.to_i).to eq(true)
      record = redis.hgetall(NewsRoot::ADMIN_NEWS_KEY)
